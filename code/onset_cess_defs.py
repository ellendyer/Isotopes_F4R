...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul  6 14:53:30 2022
...
#    ce2_years_mean = ce2_years.where(ce2_years>50,ce2_years+max_day) [conversion for
#    end of season in Jan]
...
def power_mag(freq,mag):
    ...
    power_storage1 = np.mean(mag[np.where(np.abs(freq-1)<=sens1)[0]]) 
    power_storage2 = np.mean(mag[np.where(np.abs(freq-2)<=sens2)[0]])                   
    power_storage3 = np.mean(mag[np.where(np.abs(freq-3)<=sens3)[0]])
...
def onset_cess_year(tseries,on_b,ce_b,window):
     ...
     for Y in range(int(Y1+1),Y2):
       ...
       print(np.where(C==np.max(C))[0][0],np.where(C==np.min(C))[0][0])
       if (np.where(C==np.max(C))[0][0] < 15) or (np.where(C==np.min(C))[0][0] > len(C)-15):
         inds_max = signal.argrelextrema(Cs, np.greater)[0]
         inds_min = signal.argrelextrema(Cs, np.less)[0]
         #cessation
         if len(inds_max) > 1:
       ...
       elif (np.where(C==np.min(C))[0][0]) > (np.where(C==np.max(C))[0][0]):
         ON.append(np.nan)
         OFF.append(np.nan)
         print(Y,'failed')
       ...
       for lo in xarr.lon.values:
         ...
         if np.isnan(xarr.sel(lat=la,lon=lo)).all():
           ...
           if power_ratio > 1.0:
             ...
             print('there is only one season')
