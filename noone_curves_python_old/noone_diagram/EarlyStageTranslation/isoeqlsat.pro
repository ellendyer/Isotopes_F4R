...
;
  ; CCD findgen(n) generates generates a float array of n elements with
  ; each element set to its own index as a float.
  ; I think that np.arange(n) will do the same thing from numpy=np
  ...
  ; CCD fltarr(n) creates a float array of n elements, indexed from 0
  ; with each element set to be 0.0
  ; I think that np.zeros(n) would do the same thing from numpy=np
  ...
  for n = 1, npts-1 do begin # Now build subsequent elements ...
;
...
;
    case ispec of     # Choose which alp...liq function, see alp...liq.pro
